...
class Solver(object):
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            out_src, out_cls_spks, out_spk_vec = self.D(mc_real)

            newList = []
            for i in range(len(spk_label_org)):
                newList.append(list[spk_label_org[i]])
            newArray = np.array(newList)

            d_loss_real = - torch.mean(out_src)
            d_loss_cls_spks = self.classification_loss(out_cls_spks, spk_label_org)
            d_loss_spk_vec = self.classification_loss(out_spk_vec, newArray)


            # Compute loss with fake mc feats.
            mc_fake = self.G(mc_real, spk_c_trg)
            out_src, out_cls_spks, _ = self.D(mc_fake.detach())
